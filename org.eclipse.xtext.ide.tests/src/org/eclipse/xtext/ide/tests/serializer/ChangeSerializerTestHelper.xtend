...
class ChangeSerializerTestHelper {
	...
	def void ===(Collection<ITextDocumentChange> actual, CharSequence expected) {
	...
	def <T> T contents(ResourceSet rs, String fileName, Class<T> type) {
		rs.getResource(URI.createURI(fileName), true).contents.filter(type).head
	}

	def ITextRegionAccess getTextRegionAccess(Collection<ITextDocumentChange> changes) {
		changes.filter(TextDocumentChange).map[textRegionAccess as ITextRegionAccess].head
	}
}
