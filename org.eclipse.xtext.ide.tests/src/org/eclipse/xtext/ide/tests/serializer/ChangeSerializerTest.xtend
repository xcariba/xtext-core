...
class ChangeSerializerTest {
	...
	def void testTwoChildren() {
		val fs = new InMemoryURIHandler()
		fs += "inmemory:/file1.pstl" -> '''#2 root { foo1; foo2; }'''
	...
	def void testInsertOneChild() {
		val fs = new InMemoryURIHandler()
		fs += "inmemory:/file1.pstl" -> '''#2 root { child1 { foo1; } }'''
		...
		serializer.endRecordChanges === '''
			----------------- inmemory:/file1.pstl (syntax: <offset|text>) -----------------
			#2 root {<9:17| child1 { foo1; bazz; }> }
			--------------------------------------------------------------------------------
			9 17 " child1 { foo1; }" -> " child1 { foo1; b..."
		'''
	}
	...
	def void testInsertTwoChild() {
		...
		serializer.endRecordChanges === '''
			----------------- inmemory:/file1.pstl (syntax: <offset|text>) -----------------
			#2 root {<9:17| child1 { foo1; bazz1; bazz2; }> }
			--------------------------------------------------------------------------------
			9 17 " child1 { foo1; }" -> " child1 { foo1; b..."
		'''
	}
	...
	def void testRenameLocal() {
		val fs = new InMemoryURIHandler()
		fs += "inmemory:/file1.pstl" -> '''#2 root { foo1; foo2 { ref foo1 } }'''
		...
		serializer.endRecordChanges === '''
			----------------- inmemory:/file1.pstl (syntax: <offset|text>) -----------------
			#2 root { <10:4|bazz4>; foo2 { ref <27:4|bazz4> } }
			--------------------------------------------------------------------------------
			10 4 "foo1" -> "bazz4"
			27 4 "foo1" -> "bazz4"
		'''
	...
	def void testRenameGlobal1() {
		val fs = new InMemoryURIHandler()
		fs += "inmemory:/file1.pstl" -> '''#2 root1'''
		fs += "inmemory:/file2.pstl" -> '''#2 root2 { ref root1 }'''
